...
function generateXML (data){
    ...
    channel.push({ lastBuildDate:   new Date().toISOString() });
    ...
    ifTruePush(data.pubDate, channel, { 'pubDate': data.pubDate && new Date(data.pubDate).toISOString() });
    ...
    data.items.forEach(function(item) {
        ...
        ifTruePush(item.date, item_values, { pubDate:      item.date && new Date(item.date).toISOString() });
